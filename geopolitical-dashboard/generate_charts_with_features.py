"""
Generate charts with choropleth overlay and functional filters for TEST ENVIRONMENT
"""
...
def create_radar_variations():
    """Create multiple radar chart variations for different filters."""
    df = get_data()
    variations = {}
    
    countries_list = df['Country'].unique().tolist()
    
    # Generate variations for different country selections and metrics
...
def wrap_chart_with_features(chart_title, chart_id, variations_dict, filter_config):
    """Wrap a chart with choropleth overlay and functional filters."""
    
    choropleth_fig = create_choropleth_figure()
    choropleth_html = choropleth_fig.to_html(include_plotlyjs=False, div_id="choropleth-overlay")
    
    # Convert variations to JSON-serializable format
    variations_json = {}
    for key, var in variations_dict.items():
        variations_json[key] = {
            'data': [trace.to_dict() for trace in var['data']],
            'layout': var['layout'].to_dict()
        }
    ...
    for filter_name, filter_options in filter_config.items():
        ...
        <div class="filter-group">
            ...
            <select class="filter-select" id="{chart_id}_{filter_name.lower().replace(' ', '_')}" onchange="updateChart()">
        '''
    ...
    # Create JavaScript for filter handling
    filter_js = f'''
    <script>
    ...
    function updateChart() {{
        ...
        if (window.chartVariations[key]) {{
            ...
            Plotly.react('{chart_id}-chart', chartData.data, chartData.layout);
        }}
    }}
...
def main():
    """Generate all test charts with features."""
    print("=" * 70)
