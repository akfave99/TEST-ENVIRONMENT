...
def create_page_with_functional_filters(chart_title, chart_id, variations_dict, filter_config):
    ...
    function attachChartHoverListeners() {{
        ...
        // Try using Plotly's internal event system
        // Plotly dispatches custom events on the chart div
        ...
        // Method 1: addEventListener (for standard DOM events)
        ...
        // Method 2: Try using Plotly's d3 if available
        if (typeof Plotly !== 'undefined' && Plotly.d3) {{
            console.log('Plotly.d3 is available, using it for event binding');
        ...
        // Method 3: Attach to SVG hover events directly
        setTimeout(function() {{
            const svg = chartDiv.querySelector('svg');
            if (svg) {{
                console.log('Found SVG, attaching hover listeners to SVG elements');
                const paths = svg.querySelectorAll('path[class*="country"], path[class*="geo"]');
